from transformers import AutoConfig, AutoModelForCausalLM,Qwen2Config, Qwen2Model, Qwen2ForCausalLM, CLIPVisionModel, CLIPImageProcessor
...
from app.code.utils.ocr_internal.constants import *
from app.code.core.vision_encoder.vary_b import build_vary_vit_b
...
class GOTQwenForCausalLM(Qwen2ForCausalLM):
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, DynamicCache): # More specific check for DynamicCache
                cache_length = past_key_values.get_seq_length()
                past_length = cache_length
                max_cache_length = None # DynamicCache typically doesn't have a fixed max length
            elif isinstance(past_key_values, Cache): # For other Cache types that might have get_max_length
                cache_length = past_key_values.get_seq_length()
                past_length = cache_length
